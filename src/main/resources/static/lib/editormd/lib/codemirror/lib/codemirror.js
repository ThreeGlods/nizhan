...
(function(mod) {
  ...
  function extractLineClasses(type, output) {
    if (type) for (;;) {
      ...
      if (!lineClass) break;
      type = type.slice(0, lineClass.index_bak) + type.slice(lineClass.index_bak + lineClass[0].length);
    ...
    removeLineClass: docMethodOp(function(handle, where, cls) {
      return changeLine(this, handle, where == "gutter" ? "gutter" : "class", function(line) {
        ...
        if (!cur) return false;
          ...
          var end = found.index_bak + found[0].length;
          line[prop] = cur.slice(0, found.index_bak) + (!found.index_bak || end == cur.length ? "" : " ") + cur.slice(end) || null;
        }
        return true;
      });
    }),
